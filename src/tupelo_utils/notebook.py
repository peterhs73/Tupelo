...
from tupelo_utils.shortcodes import _html_path, _file_list_grab, _titlize, _sort_time, _create_folder, _list_comp, _doc_move
...
def notebook_info_grab(notebook_list):
	...
	return (src_folder, nickname, src_nickname)
...
class FolderInfo(object):
	...
	def index_url(self):
		return _html_path(os.path.join(self.dst_folder, 'index.html'))
...
class DocIndex(object):
	...
	def inventory(self, doc_path, src_folder, dst_folder, category_list): # reducency, need to fix
		...
		if doc_name.lower() == 'readme':
			doc_name = os.path.basename(doc_dir) + "_" + "README"

...
from tupelo_utils.shortcodes import _filetype_cor, _file_list_grab, _html_path
...
class LiveEditHandle(PatternMatchingEventHandler):
	...
	def process(self, event):
		...
		self.signal.file_event.emit([
				_html_path(os.path.join(self.dst_folder, 'index.html')),
				event.src_path, 
				_html_path(os.path.join(doc_dir, 'tupelo_' + doc_base))
			])
	...
	def move_process(self, event):
		...
		self.signal.move_event.emit([
				...
				_html_path(os.path.join(doc_old_dir, 'tupelo_' + doc_old_base)), 
				_html_path(os.path.join(doc_new_dir, 'tupelo_' + doc_new_base)) # needed for log update
			])
...
class LiveEdit(QObject):
	...
	def run(self):
		...
		event_handler.signal.move_event.connect(self.move_event_change)
	...
	def event_change(self, doc_info):
		self.sig_event.emit(doc_info)

	def move_event_change(self, doc_move_info):
